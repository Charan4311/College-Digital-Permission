...
exports.getRecentScans = async (req, res) => {
  try {
    // Scanner view keeps the existing per-security-user recent scan behavior.
    // History view shows all successfully consumed gate passes.
    const historyMode = req.query.history === 'true';
    const query = historyMode
      ? { scanResult: 'VALID' }
      : { scannedByUserId: req.user.id };

    const scans = await ScanLog.find(query)
      .sort({ scannedAt: -1 })
      .limit(historyMode ? 100 : 20)
      .populate({
        path: 'requestId',
